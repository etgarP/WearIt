import React from 'react';
import { View, StyleSheet, Dimensions } from 'react-native';
import MyCard from './card'; // Ensure that this is correctly imported
import Carousel from 'react-native-reanimated-carousel';

const { width, height } = Dimensions.get('screen');

const MyCarousel = ({ setProfilePage, navigation }) => {
    // Sample data based on the provided structure
    const data = [
        {
            "username": "jane_designer",
            "__v": 0,
            "bio": "A passionate designer specializing in sustainable fashion. My creations reflect a love for nature and a commitment to ethical practices. I believe that style should never compromise our planet.",
            "designerInfo": {
                "$oid": "665a13d3f21fe10cf2321514"
            },
            "image": "https://example.com/images/jane_doe.jpg",
            "name": "Jane Doe",
            "reviews": [
                { "reviewText": 'Jane helped me choose the perfect outfit for my wedding. Her attention to detail is unmatched!', "rating": 5 },
                { "reviewText": 'Amazing work! The dress I ordered fit perfectly and looked stunning.', "rating": 4 },
                { "reviewText": 'Highly professional! Jane made the entire process enjoyable and stress-free.', "rating": 5 },
                { "reviewText": 'I love the eco-friendly materials she uses. You can really see the quality in her work!', "rating": 5 },
            ],
            "score": 92
        },
        {
            "username": "sara_designer",
            "__v": 0,
            "bio": "I am an expert in casual dresses, blending comfort with chic styles. My goal is to empower women through fashion that celebrates individuality and confidence.",
            "designerInfo": {
                "$oid": "665a13d3f21fe10cf2321515"
            },
            "image": "https://example.com/images/sara_doe.jpg",
            "name": "Sara Doe",
            "reviews": [
                { "reviewText": 'Sara is incredibly talented! The dress I ordered was not only beautiful but also super comfortable.', "rating": 4 },
                { "reviewText": 'I received so many compliments at the party! Definitely ordering again.', "rating": 5 },
                { "reviewText": 'She really knows how to tailor dresses that fit perfectly and highlight your best features.', "rating": 5 },
            ],
            "score": 95
        },
        {
            "username": "mike_the_designer",
            "__v": 0,
            "bio": "An avant-garde designer pushing the boundaries of traditional fashion. My work focuses on bold colors, innovative cuts, and reimagined silhouettes.",
            "designerInfo": {
                "$oid": "665a13d3f21fe10cf2321516"
            },
            "image": "https://example.com/images/mike_doe.jpg",
            "name": "Mike Thompson",
            "reviews": [
                { "reviewText": 'Mike’s designs are truly one-of-a-kind! I wore his outfit to a fashion show and turned heads.', "rating": 5 },
                { "reviewText": 'His use of color and fabric is incredible. I feel like a walking piece of art!', "rating": 5 },
                { "reviewText": 'I appreciate his creativity and unique vision. Definitely a designer to watch!', "rating": 4 },
            ],
            "score": 97
        }
    ];

    return (
        <View style={styles.container}>
            <Carousel
                width={width} // 90% of screen width for better appearance
                height={height*0.6}  // Adjust height based on your card size
                data={data}
                renderItem={({ item, index }) => (
                    <MyCard
                        profile={item}
                        index={index}
                        setProfilePage={setProfilePage}
                        navigation={navigation}
                    />
                )}
                mode="parallax"  // Add any carousel mode or configuration you prefer
                loop={true}  // Whether the carousel should loop
            />
        </View>
    );
};

const styles = StyleSheet.create({
    container: {
        flex: 1,
        justifyContent: "center",
        alignContent: "center"
    },

});

export default MyCarousel;
